 ...
 * both. Label is drawn rotated by the amount given in the "rotation" field (in
 * degrees).
 * 
 * <p>
...
public class RotationLabelRenderer extends AbstractShapeRenderer {
    ...
    // -1 is the standard invalid row number that item.getRow() returns and
    // we never want to match that
    ...
    /**
         * Create a new LabelRenderer. By default the field "label" is used as
         * the field name for looking up text, and no image is used.
         */
    ...
    /**
         ...
         * data field and does not draw an image.
         * 
         * @param textField
    ...
    /**
         * Create a new LabelRenderer. Draws a text label using the given text
         * data field, and draws the image at the location reported by the given
         * image data field.
         * 
         * @param textField
         *                the data field for the text label
         * @param imageField
         *                the data field for the image location. This value in
         *                the data field should be a URL, a file within the
         *                current classpath, a file on the filesystem, or null
         *                for no image. If the <code>imageField</code>
    ...
    /**
         * Rounds the corners of the bounding rectangle in which the text string
         * is rendered. This will only be seen if either the stroke or fill
         * color is non-transparent.
         * 
         * @param arcWidth
	...
	if (((arcWidth == 0) || (arcHeight == 0))
	    ...
	    ((RoundRectangle2D) m_bbox).setRoundRect(0, 0, 10, 10, arcWidth,
		    arcHeight);
	    m_arcWidth = arcWidth;
    ...
    /**
         * Sets the maximum width that should be allowed of the text label. A
         * value of -1 specifies no limit (this is the default).
         * 
         * @param maxWidth
    ...
    /**
         ...
         * field should be a URL, a file within the current classpath, a file on
         * the filesystem, or null for no image.
         * 
    ...
    /**
         ...
         * field should be a URL, a file within the current classpath, a file on
         * the filesystem, or null for no image. If the <code>imageField</code>
         * parameter is null, no images at all will be drawn.
         * 
         * @param imageField
         *                the data field for image locations, or null for no
         *                images
         */
    ...
    /**
         ...
         * images. This scaling is enforced immediately upon loading of the
         * image.
         * 
         * @param width
    ...
    /**
         * Returns a location string for the image to draw. Subclasses can
         * override this class to perform custom image selection beyond looking
         * up the value from a data field.
         * 
         * @param item
         *                the item for which to select an image to draw
         * @return the location string for the image to use, or null for no
         *         image
         */
    ...
    private String computeTextDimensions( VisualItem item,
	...
	case Constants.RIGHT:
	    w = tw
		    + size
		    * (iw + 2 * m_horizBorder + ((tw > 0) && (iw > 0) ? m_imageMargin
			    : 0));
	...
	case Constants.BOTTOM:
	    ...
	    h = th
		    + size
		    * (ih + 2 * m_vertBorder + ((th > 0) && (ih > 0) ? m_imageMargin
			    : 0));
	    break;
	default:
	...
	if (m_bbox instanceof RoundRectangle2D) {
	    ...
	    rr.setRoundRect(m_pt.getX(), m_pt.getY(), w, h, size * m_arcWidth,
		    size * m_arcHeight);
	} else {
    ...
    /**
         ...
         */
    protected static void getAlignedPoint( Point2D p,
	     VisualItem item, double w, double h,
	...
	if ((text == null) && (img == null)) {
	 ...
	 double size = item.getSize();
	 boolean useInt = 1.5 > Math.max(g.getTransform().getScaleX(), g
	...
	// do transform on graphics context instead of shape; allows rendering
	// as a rectangular shape (faster)
	 AffineTransform oldTrans = g.getTransform();
	if ((rotation > 90) && (rotation < 270)) {
	    rotation += 180;
	}
	if (rotation != 0) {
	    g.rotate(rotation * 2 * Math.PI / 360, shape.getCenterX(), shape
		    .getCenterY());
	}
	...
	if ((type == RENDER_TYPE_FILL) || (type == RENDER_TYPE_DRAW_AND_FILL)) {
	    GraphicsLib
		    .paint(g, item, shape, getStroke(item), RENDER_TYPE_FILL);
	}
	...
	if ((text != null) && (ColorLib.alpha(textColor) > 0)) {
		...
		drawString(g, fm, text.substring(start, end), useInt, x, y, tw,
			item.getBounds().getCenterX(), item.getBounds()
				.getCenterY(), 0);
		start = end + 1;
		end = text.indexOf(m_delim, start);
	    }
	    drawString(g, fm, text.substring(start), useInt, x, y, tw, item
		    .getBounds().getCenterX(), item.getBounds().getCenterY(), 0);
	}

	// draw border
	if ((type == RENDER_TYPE_DRAW) || (type == RENDER_TYPE_DRAW_AND_FILL)) {
	    GraphicsLib
		    .paint(g, item, shape, getStroke(item), RENDER_TYPE_DRAW);
	}
    ...
    /**
         * Return the graphics space transform applied to this item's shape,
         * based on rotation.
         * 
	...
	itemTransform.rotate(itemRotation * 2 * Math.PI / 360, itemRawShape
		.getCenterX(), itemRawShape.getCenterY());
	return itemTransform;
    }

    private  void drawString( Graphics2D g, FontMetrics fm,
	     String text, boolean useInt, double x,
	     double y, double w, double cx, double cy,
	    double rotation) {
    ...
    /**
         * Sets the amount of padding in pixels between the content and the
         * border of this item along the horizontal dimension.
         * 
         * @param xpad
