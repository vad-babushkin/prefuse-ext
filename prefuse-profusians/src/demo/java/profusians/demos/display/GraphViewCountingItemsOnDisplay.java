package profusians.demos.display;
...
import profusians.util.display.VisibleItemsOnDisplayCounter;

/**
 * This demo adds a paintlistener to the original graphview demo of the prefuse
 * library, which counts the number of nodes currently displayed and writes the
 * result in the upper left corner of the display. Not particular elegant yet
 * working implementation, which calls for improvments
 * 
 * @author <a href="http://jheer.org">jeffrey heer</a>
 * @author <a href="http://goosebumps4all.net">martin dudek</a>
 */
public class GraphViewCountingItemsOnDisplay extends JPanel {
    ...
    public GraphViewCountingItemsOnDisplay(Graph g, String label) {
	...
	// MAD - adding the new paintlistener which counts the current items on
        // the display
	display.addPaintListener(new MyCountListener(fdl, nodes, false, 1));
	...
	final GraphViewCountingItemsOnDisplay view = new GraphViewCountingItemsOnDisplay(
		g, label);
	...
	private GraphViewCountingItemsOnDisplay m_view;

	public GraphMenuAction(String name, String accel,
		GraphViewCountingItemsOnDisplay view) {
	    m_view = view;
	...
	public OpenGraphAction(GraphViewCountingItemsOnDisplay view) {
	    m_view = view;
    ...
    /**
         * MAD - This class counts the number of currently displayed nodes and
         * writes the result in the upper left corner. If the parameter
         * onlyCompleteOnDisplay is set true, only items which are completely on
         * the display are counted.
         * 
         * @author martin dudek
         * 
         */

    private static class MyCountListener extends VisibleItemsOnDisplayCounter {

	public MyCountListener(Layout lay, String nodeGroupName,
		boolean onlyCompleteOnDisplay, int freq) {
	    super(lay, nodeGroupName, onlyCompleteOnDisplay, freq);
	}

	public void countUpdate(Display d, Graphics2D g, int count,
		boolean realUpdate) {
