...
/**
 * A small extension of the original GraphView demo, which adds predicate based rendering etc 
 * for males to the demo. Females get bigger fonts, but males get round corner in compensation.
 * 
 * Seach for MAD (most amazing discovery) to find the 
 * relevant modifications of the original demo
 * 
...
public class GraphViewPredicateBasedRendering extends JPanel {
    ...
    public GraphViewPredicateBasedRendering(Graph g, String label) {
        ...
        DefaultRendererFactory drf = new DefaultRendererFactory(new LabelRenderer(label));
        
        //MAD - round conrners for males
        LabelRenderer maleLabelRenderer = new LabelRenderer(label);
        maleLabelRenderer.setRoundedCorner(20, 20);
        final Predicate male = ExpressionParser.predicate("INGROUP('" + nodes + "') AND gender == 'M'");
        
        drf.add(male, maleLabelRenderer);
        ...
        FontAction fonts = new FontAction(nodes, 
                FontLib.getFont("Tahoma", 17));
        
        //MAD - smaller font for males
        fonts.add(male, FontLib.getFont("Tahoma", 11));
...
//        opanel.add(overview);
    ...
    public static JFrame demo(Graph g, String label) {
        final GraphViewPredicateBasedRendering view = new GraphViewPredicateBasedRendering(g, label);
    ...
    public abstract static class GraphMenuAction extends AbstractAction {
        private GraphViewPredicateBasedRendering m_view;
        public GraphMenuAction(String name, String accel, GraphViewPredicateBasedRendering view) {
            m_view = view;
    ...
    public static class OpenGraphAction extends AbstractAction {
        ...
        public OpenGraphAction(GraphViewPredicateBasedRendering view) {
            m_view = view;
