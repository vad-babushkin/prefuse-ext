...
public class GraphEditor extends JFrame {
	...
	private Font[] fonts = new Font[]{new Font("SansSerif", 0, 10), new Font("SansSerif", 0, 14), new Font("SansSerif", 0, 20)};
	...
	public GraphEditor() {
		...
		try {
			...
			GraphEditor.Controller controller = new GraphEditor.Controller();
			...
			this.registry.setRendererFactory(new DefaultRendererFactory(nodeRenderer, edgeRenderer, (Renderer)null));
			...
			update.add(new ColorFunction() {
				...
				public Paint getFillColor(VisualItem item) {
					return (Paint)(item instanceof EdgeItem ? Color.BLACK : item.getFillColor());
				}
			});
	...
	class Controller extends ControlAdapter implements MouseListener, KeyListener, ActionListener {
		private int xDown;
		private int yDown;
		private int xCur;
		private int yCur;
			...
			if (item instanceof NodeItem) {
				...
				if (item instanceof NodeItem) {
					if (this.activeItem == null && !this.drag) {
						...
						this.addEdge(this.activeItem, item);
						item.setColor(Color.BLACK);
						item.setFillColor(Color.WHITE);
		...
		public void itemDragged(VisualItem item, MouseEvent e) {
			if (item instanceof NodeItem) {
				...
				double x = p.getX() + (double)e.getX() - (double)this.xDown;
				double y = p.getY() + (double)e.getY() - (double)this.yDown;
				item.updateLocation(x, y);
				item.setLocation(x, y);
				GraphEditor.this.activityMap.scheduleNow("update");
				this.xDown = e.getX();
		...
		public void mouseReleased(MouseEvent e) {
			...
			boolean rightClick = (e.getModifiers() & 4) > 0;
			if (rightClick) {
		...
		public void keyPressed(KeyEvent e) {
			...
			boolean modded = (modifiers & 10) > 0;
			if (Character.isLetterOrDigit(c) && !modded && src == GraphEditor.this.display && this.activeItem == null) {
				VisualItem item = this.addNode(this.xCur, this.yCur);
		...
		public void keyReleased(KeyEvent e) {
			Object src = e.getSource();
			if (src == GraphEditor.this.display.getTextEditor() && e.getKeyCode() == 10) {
				this.stopEditing();
				GraphEditor.this.activityMap.scheduleNow("update");
			}
		...
		private NodeItem addNode(int x, int y) {
			...
			item.setColor(Color.BLACK);
			item.setFillColor(Color.WHITE);
			item.updateLocation((double)x, (double)y);
			item.setLocation((double)x, (double)y);
			return item;
		}
			...
			if ("Open".equals(cmd)) {
				...
				if (chooser.showOpenDialog(GraphEditor.this.display) == 0) {
					...
					try {
						...
						JOptionPane.showMessageDialog(GraphEditor.this.display, "Sorry, an error occurred while loading the graph.", "Error Loading Graph", 0);
		...
		private void save(File f) {
			...
			try {
				...
				JOptionPane.showMessageDialog(GraphEditor.this.display, "Sorry, an error occurred while saving the graph.", "Error Saving Graph", 0);
		...
		private void setEdited(boolean s) {
			if (this.edited != s) {
				...
				if (this.saveFile == null) {
					...
					titleString = "Graph Editor - " + this.saveFile.getName() + (s ? "*" : "");
				}
